...
const HomeList = ({
  ...
  useEffect(() => {
    ...
    projCards.forEach((card) => {
      ...
      card.addEventListener('mouseleave', () => {
        projCards.forEach((c) => c.classList.remove('hovered', 'not-hovered'))
      })
    })

    // Scroll handler
    const handleScroll = (e: WheelEvent) => {
      // 🧠 Prevent scroll if dropdown is open
  ...
  }, [currentIndex, isScrolling, setprojectIndex])
  ...
  return (
    <div className='home-list-cards flex absolute bottom-[2%] right-[2%] gap-[14px]'>
      {projects?.map((project: Project, index) => (
        ...
        >
          <img
            className='inset-0 h-full w-full object-cover rounded-[2px]'
            src={project.image}
          />
        </button>
      ))}
